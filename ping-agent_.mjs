...
// Cache for the Valve SDR relay list (see the /sdr-relays route below) —
// appid -> { at, pops }. Kept in memory only; refetched after SDR_TTL_MS.
...
// ── TCP-handshake fallback ping ──────────────────────────────────────────
// Some hardened servers (game relays, old dedicated datacenter gateways)
// silently drop ICMP echo requests but still complete a TCP handshake (or at
// least answer with a same-speed RST). Timing that handshake gives a real
// RTT even when `ping` reports 100% loss. We only trust it as a genuine
// answer on "connect" or "ECONNREFUSED" (both mean the remote IP is alive
// and responded); a hang/timeout still counts as unreachable (-1).
function tcpPingOnce(host, port, timeoutMs) {
  return new Promise((resolve) => {
    ...
    let socket;
    try {
      socket = createConnection({ host, port, timeout: timeoutMs });
    } catch {
      return resolve(-1);
    }
    socket.once("connect", () => finish(Date.now() - start));
    socket.once("error", (err) => {
      if (err && err.code === "ECONNREFUSED") finish(Date.now() - start);
      else finish(-1);
    });
...
function pingOne(host) {
  return new Promise((resolve) => {
    ...
    execFile("ping", args, { timeout: TIMEOUT_MS + 500 }, async (err, stdout) => {
      ...
      // ICMP failed or gave no reply (blocked/filtered) — try a TCP
      // handshake instead before giving up.
      resolve(await tcpPingBest(host));
    });
  });
}
...
<html lang="fa" dir="rtl">
  ...
  // ── Valve SDR relay list proxy (REAL CS2/Dota2 latency) ─────────────
  // Steam Datagram Relay (SDR) node IPs ARE public and pingable — the game
  // client itself measures ping to these exact IPs before matchmaking. We
  // fetch Valve's own config from Node (no CORS restriction, and Steam's
  // API has no CORS headers for browsers anyway) and hand back one relay
  // IP per POP (point of presence / region). The dashboard then pings the
  // ACTUAL relay your game would use — instead of an unrelated cloud
  // region used as a rough geographic stand-in — so numbers match in-game
  // ping (e.g. Dubai/dxb) far more closely.
  if (req.method === "GET" && req.url?.startsWith("/sdr-relays")) {
    const u = new URL(req.url, `http://127.0.0.1:${PORT}`);
    const appid = Number(u.searchParams.get("appid") || 730);
    const cached = sdrCache.get(appid);
    if (cached && Date.now() - cached.at < SDR_TTL_MS) {
      res.writeHead(200, { ...CORS, "Content-Type": "application/json" });
      return res.end(JSON.stringify({ pops: cached.pops }));
    }
    fetch(`https://api.steampowered.com/ISteamApps/GetSDRConfig/v1?appid=${appid}`, {
      headers: { "User-Agent": "Mozilla/5.0" },
    })
      .then((r) => r.json())
      .then((json) => {
        const rawPops = (json && json.pops) || {};
        const pops = {};
        for (const [popid, p] of Object.entries(rawPops)) {
          const relays = Array.isArray(p?.relays) ? p.relays : [];
          const relay = relays.find((x) => x && x.ipv4);
          if (!relay) continue;
          pops[popid] = { desc: p.desc || popid.toUpperCase(), ip: String(relay.ipv4) };
        }
        sdrCache.set(appid, { at: Date.now(), pops });
        res.writeHead(200, { ...CORS, "Content-Type": "application/json" });
        res.end(JSON.stringify({ pops }));
      })
      .catch((e) => {
        // Serve stale cache (if any) rather than nothing when Steam is down.
        if (cached) {
          res.writeHead(200, { ...CORS, "Content-Type": "application/json" });
          return res.end(JSON.stringify({ pops: cached.pops, stale: true }));
        }
        res.writeHead(502, { ...CORS, "Content-Type": "application/json" });
        res.end(JSON.stringify({ error: String(e?.message || e) }));
      });
